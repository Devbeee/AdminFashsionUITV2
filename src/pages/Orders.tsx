...
export function Orders() {
  ...
  const optionTemplate = (option: SortType) => {
    return (
  ...
  const sortOptionTemplate = (option: SortType) => {
    return optionTemplate(option)
  }
  const selectedOptionTemplate = (option: SortType) => {
    return optionTemplate(option)
  }
  const sortOptions = [
    {
            ...
            >
                  ...
                  <div className={`${data.deletedAt && 'line-through opacity-50'}`}>
                    {
                      <div
                        className={` text-center ${
                          [OrderStatus.Delivered, OrderStatus.Confirmed, OrderStatus.Delivering].includes(
                            data?.orderStatus
                          )
                            ? 'text-green-500'
              ...
              <Column
                ...
                body={(data: IOrderReturn) =>
                  data.paidAt ? (
                    <div
                      className={`text-center flex flex-col items-center ${data.deletedAt && 'line-through opacity-50'}`}
                    >
